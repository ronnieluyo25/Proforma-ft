  ...
  const head = `
    <thead class="no-break">
      <tr>
        ...
        <th>Modalidad</th><th>Horas</th><th>Precio x Hora</th><th>Importe</th>
      </tr>
    </thead>`;
  const body = rows.map(r => `
    <tr>
      <td>${r.fecha}</td><td>${r.dia}</td><td>${r.tutor}</td><td>${r.modalidad}</td>
      <td>${r.horas.toFixed(2)}</td><td>S/ ${r.precioHora.toFixed(2)}</td><td>S/ ${r.importe.toFixed(2)}</td>
    </tr>`).join('');
  const totH = rows.reduce((a,b)=>a+b.horas,0);
  const totI = rows.reduce((a,b)=>a+b.importe,0);
  const foot = `
    <tfoot>
      <tr><td colspan="4">TOTAL HORAS</td><td>${totH.toFixed(2)}</td><td>TOTAL</td><td>S/ ${totI.toFixed(2)}</td></tr>
    </tfoot>`;
...
function generarProforma(){
  ...
  const h      = toNum($('#horasClase').value);
  const p      = toNum($('#precioHora').value);
  const n      = Number($('#secciones').value);
...
function descargarPDF(){
  ...
  // Ancho útil de A4 con márgenes 10mm
  const A4_INNER_PX = 716;

  const opt = {
    margin: [10, 10, 16, 10],   // +6mm abajo para evitar corte inferior
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  DIAS.forEach(d => {
    ...
    l.innerHTML = `<input type="checkbox" class="accent-blue-600" value="${d}"> ${d}`;
    cont.appendChild(l);
  });
