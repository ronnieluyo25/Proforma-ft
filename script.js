...
function renderDetalle(rows){
  const t=$('#tabla');
  t.innerHTML=`<thead><tr><th>Fecha</th><th>Día</th><th>Tutor</th><th>Modalidad</th><th>Horas</th><th>Precio x Hora</th><th>Importe</th></tr></thead>`;
  ...
  t.innerHTML+=`<tbody>${body}</tbody><tfoot><tr><td colspan="4">TOTAL HORAS</td><td>${totH.toFixed(2)}</td><td>TOTAL</td><td>S/ ${totI.toFixed(2)}</td></tr></tfoot>`;
}
...
function generarProforma(){
  ...
  while(filas.length<n){if(dias.includes(DIAS[d.getDay()])){filas.push({fecha:toDMY(d),dia:DIAS[d.getDay()],tutor,modalidad:mod,horas:h,precioHora:p,importe:h*p});}d.setDate(d.getDate()+1);}
  renderDetalle(filas);$('#phAlumno').textContent=alumno;$('#phTutor').textContent=tutor;$('#phPaquete').textContent=`${n} sesiones - ${(n*h).toFixed(2)}h`;$('#exportArea').classList.remove('hidden');$('#pdfBtn').disabled=false;
}

function descargarPDF(){
  const area=$('#exportArea');
  area.style.overflow='visible';
  const opt={margin:[10,10,10,10],filename:'Proforma_FriendTeacher.pdf',
